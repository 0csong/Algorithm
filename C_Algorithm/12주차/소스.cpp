...
struct cmp_knapsack { //우선순위큐에서 bound 값이 큰 순서대로 가져옴
	bool operator()(node_knapsack a, node_knapsack b) {
...
void bound_knapsack(node_knapsack* u) { //노드 u의 bound값을 계산
	...
	if (u->weight > W) //노드 u의 무게가 베낭의 허용용량 초과
		...
		if (j <= N_ITEMS)
			u->bound = u->bound + (W - totweight) * p[j] /w[j] ;
...
struct node_tsp {
	...
	int setA_last;//아직까지 방문하지 못한 도시를 방문시 마지막 방문 도시
...
void last_path(node_tsp* u) {
	int new_id = u->setA[u->setA_last];
	u->distance += mat[u->path[u->path_last]][new_id];
	u->distance += mat[new_id][0];
	u->path_last++;
	u->path[u->path_last] = new_id;
	u->setA_last--;
	u->bound = u->distance;
	//a->bsetA[new_id]= true;
}
...
외판원 문제 최고우선 검색
삽입:distance=0, bound=19, [0 ]
		삭제:distance=0, bound=19, [0 ]
삽입:distance=14, bound=26, [0 1 ]
삽입:distance=4, bound=19, [0 2 ]
삽입:distance=10, bound=33, [0 3 ]
삽입:distance=20, bound=43, [0 4 ]
		삭제:distance=4, bound=19, [0 2 ]
삽입:distance=9, bound=19, [0 2 1 ]
삽입:distance=20, bound=42, [0 2 4 ]
삽입:distance=11, bound=32, [0 2 3 ]
		삭제:distance=9, bound=19, [0 2 1 ]
삽입:distance=37, bound=37, [0 2 1 3 4 ]
삽입:distance=31, bound=31, [0 2 1 4 3 ]
		삭제:distance=14, bound=26, [0 1 ]
삽입:distance=21, bound=38, [0 1 4 ]
삽입:distance=21, bound=29, [0 1 2 ]
삽입:distance=22, bound=45, [0 1 3 ]
		삭제:distance=21, bound=29, [0 1 2 ]
삽입:distance=52, bound=52, [0 1 2 4 3 ]
삽입:distance=48, bound=48, [0 1 2 3 4 ]
		삭제:distance=11, bound=32, [0 2 3 ]
		삭제:distance=10, bound=33, [0 3 ]
		삭제:distance=20, bound=42, [0 2 4 ]
		삭제:distance=20, bound=43, [0 4 ]
best tsp: distance=31, bound=31, [0 2 1 4 3 ]

C:\Users\nicky\Desktop\3-2학기\알고리즘\12주차\Project1\Debug\Project1.exe(프로세스 1652개)이(가) 종료되었습니다(코드: 0개).
이 창을 닫으려면 아무 키나 누르세요...
외판원 문제 틀림 이유는 잘모르겟습니다
